...
use super::circumcenters::circumcenters;
use super::delaunay_from::delaunay_from;
use super::edges::edges;
use super::find::find;
use super::hull::hull;
use super::mesh::mesh;
use super::neighbors::neighbors;
use super::polygons::polygons;
use super::triangles::triangles;
use super::urquhart::urquhart;
  ...
  F: Float + FloatConst + FromPrimitive + 'static,
{
  fn delaunay(points: Vec<[F; 2]>) -> Option<DelaunayReturn<F>> {
    match delaunay_from(points) {
      Some(delaunay) => {
        let tri: Vec<[usize; 3]> = triangles(delaunay);
        let e: Vec<[usize; 2]> = edges(tri, points);
        let pr = polygons(circumcenters(tri, points), tri, points);
        let polys = pr.polygons;
